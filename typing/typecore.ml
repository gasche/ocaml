  ...
  | Pexp_letop { let_ = slet; ands = sands; body = sbody } ->
      ...
      let rec loop spat_acc ty_acc sands =
        ...
        | { pbop_pat = spat; pbop_attributes; _ } :: rest ->
            ...
            let spat_acc = Ast_helper.Pat.tuple ~loc [spat_acc; { spat with ppat_attributes = spat.ppat_attributes @ pbop_attributes; } ] in
...
and type_andops env sarg sands expected_ty =
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | [] ->
        Builtin_attributes.warning_scope ~ppwarning:false let_sarg.pbop_attributes
          (fun () ->
             type_expect env let_sarg.pbop_expr (mk_expected expected_ty), []
          )
    | { pbop_op = sop; pbop_expr = sexp; pbop_loc = loc; pbop_attributes } :: rest ->
        ...
        let exp =
          Builtin_attributes.warning_scope ~ppwarning:false pbop_attributes (fun () ->

...
et structure_item_desc =
  ...
  | Tstr_open de override_flag * Path.t * Longident.t loc * attribute list
  ...
  | Tstr_include de module_expr * Types.signature * attribute list
...
et signature_item_desc =
  ...
  | Tsig_open de override_flag * Path.t * Longident.t loc * attribute list
  | Tsig_include de module_type * Types.signature * attribute list
...
soit iter_pattern_desc f = fonction
  ...
  | Tpat_record (lbl_pat_list, _) ->
      List.iter (fonc (_, lbl, pat) -> f pat) lbl_pat_list
...
soit map_pattern_desc f d =
  ...
  | Tpat_record (lpats, closed) ->
      Tpat_record (List.map (fonc (lid, l,p) -> lid, l, f p) lpats, closed)
...
soit idents = ref([]: (Ident.t * string loc) list)

soit rec bound_idents pat =
...
soit pat_bound_idents pat =
  idents := []; bound_idents pat; soit res = !idents dans idents := []; res

soit rev_let_bound_idents_with_loc bindings =
  idents := [];
  List.iter (fonc vb -> bound_idents vb.vb_pat) bindings;
  soit res = !idents dans idents := []; res

soit let_bound_idents_with_loc pat_expr_list =
  List.rev(rev_let_bound_idents_with_loc pat_expr_list)

soit rev_let_bound_idents pat = List.map fst (rev_let_bound_idents_with_loc pat)
soit let_bound_idents pat = List.map  fst (let_bound_idents_with_loc pat)

soit alpha_var env id = List.assoc id env

soit rec alpha_pat env p = filtre p.pat_desc avec
| Tpat_var (id, s) -> (* note the ``Not_found'' case *)
    {p avec pat_desc =
     essaie Tpat_var (alpha_var env id, s) avec
     | Not_found -> Tpat_any}
| Tpat_alias (p1, id, s) ->
    soit new_p =  alpha_pat env p1 dans
    début essaie
      {p avec pat_desc = Tpat_alias (new_p, alpha_var env id, s)}
    avec
    | Not_found -> new_p
    fin
| d ->
    {p avec pat_desc = map_pattern_desc (alpha_pat env) d}

soit mkloc = Location.mkloc
soit mknoloc = Location.mknoloc

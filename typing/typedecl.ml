...
let check_well_founded env loc path to_check visited ty0 =
  let rec check parents trace ty =
    ...
    | Tconstr(p, tyl, _) ->
        ...
        if to_check then List.iter (check_sub parents trace ty) tyl;
        begin match Ctype.try_expand_once_opt env ty with
        | ty' -> check_expand parents trace ty ty'
        | exception Ctype.Cannot_expand ->
            if not to_check then List.iter (check_sub parents trace ty) tyl
        end
    | _ ->
        Btype.iter_type_expr (check_sub parents trace ty) ty
  and check_sub parents trace outer_ty inner_ty =
    check parents (Contains (outer_ty, inner_ty) :: trace) inner_ty
  and check_expand parents trace ty ty' =

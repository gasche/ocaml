...
let transl_with_constraint id row_path ~sig_env ~sig_decl ~outer_env sdecl =
  ...
  let tparams = make_params outer_env sdecl.ptype_params in
  let params = List.map (fun (cty, _) -> cty.ctyp_type) tparams in
  ...
  let constraints = List.map
    (function (ty, ty', loc) ->
       try
         let cty = transl_simple_type outer_env false ty in
         let cty' = transl_simple_type outer_env false ty' in
         let ty = cty.ctyp_type in
         let ty' = cty'.ctyp_type in
         Ctype.unify outer_env ty ty';
         (cty, cty', loc)
       with Ctype.Unify tr ->
         raise(Error(loc, Inconsistent_constraint (outer_env, tr))))
    sdecl.ptype_cstrs
  in
  let no_row = not (is_fixed_type sdecl) in
  let (tman, man) =  match sdecl.ptype_manifest with
      None -> None, None
    | Some sty ->
        let cty = transl_simple_type outer_env no_row sty in
        Some cty, Some cty.ctyp_type
  in
  let priv =
  ...
  (* Note: [type_kind] comes from [sig_decl];
     it is valid in [sig_env] but may not be valid in [outer_env] *)
  ...
  in
  let arity = List.length params in
  (* the use of [type_kind] from [sig_decl] means that
     [new_sig_decl] is only valid in [sig_env] *)
  let new_sig_decl =
    { type_params = params;
      type_arity = arity;
      type_kind;
  ...
  | Some p -> set_fixed_row sig_env loc p new_sig_decl
  end;
  ...
  let new_type_variance =
    try Typedecl_variance.compute_decl
          sig_env ~check:true new_sig_decl (Typedecl_variance.variance_of_sdecl sdecl)
    with Typedecl_variance.Error (loc, err) ->
      raise (Error (loc, Variance err)) in
  let new_type_immediate =
    (* Typedecl_immediacy.compute_decl never raises *)
    Typedecl_immediacy.compute_decl sig_env new_sig_decl in
  let new_type_separability =
    try Typedecl_separability.compute_decl sig_env new_sig_decl

...
(* Given a matrix of non-empty rows
   ...
   where a "simple pattern" starts with either the catch-all pattern omega (_) or
   ...
   becomes
     (_, { row = r1; seen = s1 ++ {x} })
     (Some _, { row = r2; seen = s2 ++ {y} })
     (None, { row = r3; seen = s3 ++ {x, y} })
     (Some x, { row = r4; seen = s4 })
     (None, { row = r4; seen = s4 ++ {x} })
 *)

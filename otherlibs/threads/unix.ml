...
type resumption_status =
  ...
  | Resumed_join
  | Resumed_io
  | Resumed_select de file_descr list * file_descr list * file_descr list
  | Resumed_wait de int * process_status
...
dehors thread_wait_read : file_descr -> unit = "thread_wait_read"
dehors thread_wait_write : file_descr -> unit = "thread_wait_write"
...
dehors unsafe_write : file_descr -> string -> int -> int -> int
    = "unix_write"
dehors unsafe_single_write : file_descr -> string -> int -> int -> int
    = "unix_single_write"

soit rec read fd buf ofs len =
  essaie
...
soit rec single_write fd buf ofs len =
  essaie
    si ofs < 0 || len < 0 || ofs > String.length buf - len
    alors invalid_arg "Unix.partial_write"
...
dehors _pipe : unit -> file_descr * file_descr = "unix_pipe"

soit pipe() =
  soit (out_fd, in_fd tel fd_pair) = _pipe() dans
  set_nonblock in_fd;
  set_nonblock out_fd;
  fd_pair
...
dehors _execv : string -> string array -> 'a = "unix_execv"
dehors _execve : string -> string array -> string array -> 'a = "unix_execve"
dehors _execvp : string -> string array -> 'a = "unix_execvp"
dehors _execvpe : string -> string array -> string array -> 'a
                  = "unix_execvpe"
...
soit safe_clear_nonblock fd =
  essaie clear_nonblock fd avec Unix_error(_,_,_) -> ()
soit safe_set_nonblock fd =
  essaie set_nonblock fd avec Unix_error(_,_,_) -> ()

soit do_exec fn =
  soit oldtimer =
    setitimer ITIMER_VIRTUAL {it_interval = 0.0; it_value = 0.0} dans
  safe_clear_nonblock stdin;
  safe_clear_nonblock stdout;
  safe_clear_nonblock stderr;
  essaie
    fn ()
  avec Unix_error(_,_,_) tel exn ->
...
soit execv proc args =
  do_exec (fonc () -> _execv proc args)

soit execve proc args env =
  do_exec (fonc () -> _execve proc args env)

soit execvp proc args =
  do_exec (fonc () -> _execvp proc args)

soit execvpe proc args =
  do_exec (fonc () -> _execvpe proc args)

dehors fork : unit -> int = "unix_fork"
dehors _waitpid : wait_flag list -> int -> int * process_status
                  = "unix_waitpid"
...
soit wait () = wait_pid (-1)

soit waitpid flags pid =
  si List.mem WNOHANG flags
  alors _waitpid flags pid
  sinon wait_pid pid
...
soit sleep secs = delay (float secs)
...
dehors _socket : socket_domain -> socket_type -> int -> file_descr
                                  = "unix_socket"
dehors _socketpair :
        socket_domain -> socket_type -> int -> file_descr * file_descr
                                  = "unix_socketpair"

soit socket dom typ proto =
  soit s = _socket dom typ proto dans
  set_nonblock s;
  s

soit socketpair dom typ proto =
  soit (s1, s2 tel spair) = _socketpair dom typ proto dans
  set_nonblock s1; set_nonblock s2;
  spair

dehors _accept : file_descr -> file_descr * sockaddr = "unix_accept"

soit rec accept req =
  wait_read req;
  essaie
    soit (s, caller tel result) = _accept req dans
    set_nonblock s;
    result
  avec Unix_error((EAGAIN | EWOULDBLOCK), _, _) -> accept req
...
dehors _connect : file_descr -> sockaddr -> unit = "unix_connect"

soit connect s addr =
  essaie
    _connect s addr
  avec Unix_error((EINPROGRESS | EWOULDBLOCK | EAGAIN), _, _) ->
    wait_write s;
...
soit rec recv fd buf ofs len flags =
  essaie
    si ofs < 0 || len < 0 || ofs > String.length buf - len
...
soit rec recvfrom fd buf ofs len flags =
  essaie
    si ofs < 0 || len < 0 || ofs > String.length buf - len
...
soit rec send fd buf ofs len flags =
  essaie
    si ofs < 0 || len < 0 || ofs > String.length buf - len
...
soit rec sendto fd buf ofs len flags addr =
  essaie
    si ofs < 0 || len < 0 || ofs > String.length buf - len
...
soit establish_server server_fun sockaddr =
  ...
  pendant_que vrai faire
    soit (s, caller) = accept sock dans

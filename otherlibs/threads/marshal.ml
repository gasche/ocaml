...
soit to_buffer buff ofs len v flags =
  si ofs < 0 || len < 0 || ofs + len > String.length buff
  alors invalid_arg "Marshal.to_buffer: substring out of bounds"
  sinon to_buffer_unsafe buff ofs len v flags

soit to_buffer' ~buf ~pos ~len v ~mode = to_buffer buf pos len v mode
...
soit from_channel = Pervasives.input_value

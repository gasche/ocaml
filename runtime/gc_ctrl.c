...
static uintnat get_ramp_up_suspended_words(void) {
  return (Caml_state->current_rampup_allocated_words_diff
...
value caml_gc_ramp_up(value callback, uintnat *out_suspended_words) {
    ...
    caml_gc_message(0x40, "Entering a GC ramp-up phase.\n");

    intnat rampup_already = (Caml_state->gc_policy & CAML_GC_RAMP_UP);
    if (!rampup_already)
      Caml_state->gc_policy = (Caml_state->gc_policy | CAML_GC_RAMP_UP);

    /* Save the suspended words of a potential outer phase,
       and start a new rampup phase. */
    uintnat suspended_words_outer = get_ramp_up_suspended_words();
    if (!rampup_already) CAMLassert(suspended_words_outer == 0);
    ...
    *out_suspended_words = suspended_words_inner;
    ...
    return caml_raise_if_exception(res_or_exn);
}
...
CAMLprim value caml_ml_gc_ramp_up(value callback) {
  ...
  v = caml_gc_ramp_up(callback, &deferred_words);

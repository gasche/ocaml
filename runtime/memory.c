...
/* The naming of the [caml_atomic_cas_*] primitives is slightly different
   from other [caml_atomic_*] primitives, because a [caml_atomic_cas_field]
   at this type was exposed in pre-5.0 Multicore OCaml (even without
   CAML_INTERNALS). For now we keep it around for backward-compatibility. We
   call [caml_atomic_cas_field_boxed] the version that corresponds to the
   other [caml_atomic_foo_field] functions. */
...
CAMLprim value caml_atomic_cas_field_boxed (
  value obj, value field, value oldval, value newval)
{
  return Val_bool(caml_atomic_cas_field(obj, Long_val(field), oldval, newval));
}

...
and pp_output_char state c =
  let s = " " in
  s.[0] <- c;
  state.pp_output_function s 0 1
...
let ifprintf x (fmt, _) = make_printf (fun _ _ -> ()) x End_of_acc fmt

let printf (fmt, _) = make_printf output_acc std_formatter End_of_acc fmt
let eprintf (fmt, _) = make_printf output_acc err_formatter End_of_acc fmt

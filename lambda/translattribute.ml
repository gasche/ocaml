...
let get_construct_from_exp =
  let open Parsetree in
  function
    | { pexp_desc = Pexp_construct ({ txt = Longident.Lident constr }, None) } ->
        Result.Ok constr
    | _ -> Result.Error ()
...
let get_tailcall_attribute e =
  ...
  let tailcall_attribute = match tailcalls with
    ...
    | {Parsetree.attr_name = {txt; loc}; attr_payload = payload} :: r ->
        ...
        | Ok None -> Should_be_tailcall (* [@tailcall] *)
        | Ok (Some true) -> Should_be_tailcall (* [@tailcall true] *)
        | Ok (Some false) -> Should_not_be_tailcall (* [@tailcall false] *)
        | Error () ->

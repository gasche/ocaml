...
(letrec
  (map/0
     (function f/0 param/0 tail_mod_cons
       (if param/0
         (let
           (block/0 = (makemutable 0 (apply f/0 (field_imm 0 param/0)) 24029))
           (seq (apply map_dps/0 block/0 1 f/0 (field_imm 1 param/0))
             block/0))
         0))
    map_dps/0
      (function dst/0 offset/0[int] f/1 param/1 tail_mod_cons
        (if param/1
          (let
            (block0_arg0/0 = (apply f/1 (field_imm 0 param/1))
             block/1 = (makemutable 0 block0_arg0/0 24029))
            (seq (setfield_ptr(heap-init)_computed dst/0 offset/0 block/1)
              (apply map_dps/0 block/1 1 f/1 (field_imm 1 param/1) tailcall)))
       ...
       (if param/2
         (let (*match*/0 =a (field_imm 0 param/2))
           (makeblock 0
             (let
               (block/2 =
                  (makemutable 0 (apply f/2 (field_imm 0 *match*/0)) 24029))
               (seq
                 (apply rec_map_dps/0 block/2 1 f/2 (field_imm 1 *match*/0))
                 block/2))))
         0))
    rec_map_dps/0
        ...
        (if param/3
          (let
            (*match*/1 =a (field_imm 0 param/3)
             block1_arg0/0 = (apply f/3 (field_imm 0 *match*/1))
             block/3 = (makemutable 0 block1_arg0/0 24029))
            (seq
              (setfield_ptr(heap-init)_computed dst/1 offset/1
                (makeblock 0 block/3))
              (apply rec_map_dps/0 block/3 1 f/3
                (field_imm 1 *match*/1) tailcall)))
...
(letrec
  (trip/0
     (function param/4 tail_mod_cons
       (if param/4
         (let (x/0 =a (field_imm 0 param/4))
           (makeblock 0 (makeblock 0 (*,int) x/0 0)
             (makeblock 0 (makeblock 0 (*,int) x/0 1)
               (let
                 (block/4 = (makemutable 0 (makeblock 0 (*,int) x/0 2) 24029))
                 (seq (apply trip_dps/0 block/4 1 (field_imm 1 param/4))
                   block/4)))))
         0))
    trip_dps/0
      (function dst/2 offset/2[int] param/5 tail_mod_cons
        (if param/5
          (let
            (x/1 =a (field_imm 0 param/5)
             block0_arg0/1 = (makeblock 0 (*,int) x/1 0)
             block1_arg0/1 = (makeblock 0 (*,int) x/1 1)
             block2_arg0/0 = (makeblock 0 (*,int) x/1 2)
            ...
            (seq
              ...
              (apply trip_dps/0 block/5 1 (field_imm 1 param/5) tailcall)))
...
(letrec
  (effects/0
     (function f/4 param/6 tail_mod_cons
       (if param/6
         (let (*match*/2 =a (field_imm 0 param/6))
           (makeblock 0 (apply f/4 (field_imm 0 *match*/2))
             (let
               (block/6 =
                  (makemutable 0 (apply f/4 (field_imm 1 *match*/2)) 24029))
               (seq (apply effects_dps/0 block/6 1 f/4 (field_imm 1 param/6))
                 block/6))))
         0))
    effects_dps/0
        ...
        (if param/7
          (let
            (*match*/3 =a (field_imm 0 param/7)
             block0_arg0/2 = (apply f/5 (field_imm 0 *match*/3))
             block1_arg0/2 = (apply f/5 (field_imm 1 *match*/3))
            ...
            (seq
              ...
              (apply effects_dps/0 block/7 1 f/5
                (field_imm 1 param/7) tailcall)))
...
(letrec
  (map_stutter/0
     (function f/6 xs/0 tail_mod_cons
       (makeblock 0 (apply f/6 0)
         (if xs/0
           (let
             (block/8 =
                (makemutable 0 (apply f/6 (makeblock 0 (field_imm 0 xs/0)))
                  24029))
             (seq (apply map_stutter_dps/0 block/8 1 f/6 (field_imm 1 xs/0))
               block/8))
           0)))
    map_stutter_dps/0
      (function dst/4 offset/4[int] f/7 xs/1 tail_mod_cons
        (let
          ...
          (seq (setfield_ptr(heap-init)_computed dst/4 offset/4 block/9)
            (if xs/1
              (let
                (block0_arg0/4 = (apply f/7 (makeblock 0 (field_imm 0 xs/1)))
                 block/10 = (makemutable 0 block0_arg0/4 24029))
                (seq (setfield_ptr(heap-init)_computed block/9 1 block/10)
                  (apply map_stutter_dps/0 block/10 1 f/7
                    (field_imm 1 xs/1) tailcall)))
...
(letrec
  (smap_stutter/0
     (function f/8 xs/2 n/0[int] tail_mod_cons
       (if (== n/0 0) 0
         (makeblock 0 (apply f/8 0)
           (let
             (v/0 = (apply f/8 (makeblock 0 (field_imm 0 xs/2)))
              block/11 = (makemutable 0 v/0 24029))
             (seq
               (apply smap_stutter_dps/0 block/11 1 f/8
                 (apply (field_imm 1 xs/2) 0) (- n/0 1))
               block/11)))))
    smap_stutter_dps/0
      (function dst/5 offset/5[int] f/9 xs/3 n/1[int] tail_mod_cons
        (if (== n/1 0) (setfield_ptr(heap-init)_computed dst/5 offset/5 0)
          (let
            (block0_arg0/5 = (apply f/9 0)
             v/1 = (apply f/9 (makeblock 0 (field_imm 0 xs/3)))
            ...
            (seq
              ...
              (apply smap_stutter_dps/0 block/12 1 f/9
                (apply (field_imm 1 xs/3) 0) (- n/1 1) tailcall))))))

...
(* Test that combined constructors preserve the TMC-specified evaluation order *)
let [@tail_mod_cons] rec doubling_map f = function
| [] -> []
| x :: xs -> f true x :: f false x :: doubling_map f xs

let () =
  assert
    ((doubling_map (fun b n -> Printf.printf "%d %B\n%!" n b; (n, b)) [1; 2; 3])

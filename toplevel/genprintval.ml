          ...
          | Tconstr (path, [ty_arg], _)
            when Path.same path Predef.path_lazy_t ->
             ...
             (* Lazy values are represented in three possible ways:
                1. a lazy thunk that is not yet forced has tag Obj.lazy_tag
                2. a lazy thunk that has been forced has tag Obj.forward_tag;
                   its first field is the forced result, which we can print
                3. creating a lazy thunk from a value may produce a special
                   representation where the value is directly exposed, without
                   the Obj.forward_tag (if its own tag is not ambiguous, that is
                   neither lazy_tag nor forward_tag)

                Note that using Lazy.is_val and Lazy.force is unsafe
                in the current code, because it uses the Obj.* functions rather
                than the O.* functions of the functor argument, and would thus
                crash if called from the toplevel (debugger/printval instantiates
                Genprintval.Make with an Obj module talking over a socket).
              *)

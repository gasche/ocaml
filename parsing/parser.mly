...
module_declaration_body:
  ...
  | LPAREN UIDENT COLON module_type RPAREN module_declaration_body
      { _mkmty(Pmty_functor(_mkrhs($2, 2), Some $4, $6)) }
  | LPAREN RPAREN module_declaration_body
      { _mkmty(Pmty_functor(_mkrhs2("*", 1, 2), None, $3)) }
;
module_declaration:
...
class_sig_fields:
    /* empty */                                 { [] }
| class_sig_fields class_sig_field     { $2 :: _text_csig(2) @ $1 }
;
class_sig_field:
...
tag_field:
    name_tag OF opt_ampersand amper_type_list attributes
      { Rtag (_mkrhs($1, 1), add_info_attrs (_symbol_info ()) $5,
               $3, List.rev $4) }
  | name_tag attributes
      { Rtag (_mkrhs($1, 1), add_info_attrs (_symbol_info ()) $2, true, []) }
;
opt_ampersand:
    AMPERSAND                                   { true }
  | /* empty */                                 { false }
...
field:
  label COLON poly_type_no_attr attributes
    { Otag (_mkrhs($1, 1), add_info_attrs (_symbol_info ()) $4, $3) }
;
...
field_semi:
  label COLON poly_type_no_attr attributes SEMI attributes
    { let info =
        ...
        | None -> _symbol_info()
      in
      ( Otag (_mkrhs($1, 1), add_info_attrs info ($4 @ $6), $3)) }
;

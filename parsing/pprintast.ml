...
and binding_op ctxt f x =
  match x.pbop_pat, x.pbop_expr with
  | {ppat_desc = Ppat_var { txt=pvar; _ }; ppat_attributes = []; _},
    {pexp_desc = Pexp_ident { txt=Lident evar; _}; pexp_attributes = []; _}
       when pvar = evar ->
     pp f "@[<2>%s %a%s@]" x.pbop_op.txt (attributes ctxt) x.pbop_attributes evar
  | pat, exp ->
     pp f "@[<2>%s %a%a@;=@;%a@]"
       x.pbop_op.txt (attributes ctxt) x.pbop_attributes (pattern ctxt) pat (expression ctxt) exp
